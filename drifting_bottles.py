...
# 随机捞漂流瓶
async def pick_drifting_bottles_radom(
    websocket, user_id: int, group_id: int, text: str
):
    conn = sqlite3.connect("bot.db")
    cur = conn.cursor()
    try:
    ...
    if row is None:
        ...
        text = f"{get_user_name(user_id,group_id)}捞到了一个{get_user_name(row[1],row[2])}于{timestamp_to_date(row[5])}在{GetGroupName(row[2])}丢的漂流瓶,标识ID为:{row[0]},内容为:{row[3]}"
        # user_id, group_id, text, time
        all_comment = load_comment(row[0])
        for comment in all_comment:
            text = (
                text
                + f"\n{get_user_name(comment[0],comment[1])}于{timestamp_to_date(comment[3])}在{GetGroupName(comment[1])}评论:{comment[2]}"
            )
        await say_and_echo(
            websocket,
            text,
...
def dump_comment(uuid: str, user_id: int, group_id: int, text: str):
    ...
    try:
        cur.execute(
            ...
            (user_id, group_id, text, time.time()),
        )
        conn.commit()
    except sqlite3.OperationalError:
        logging.info("数据库表不存在,正在创建表drifting_bottles_comments")
        cur.execute(
            "CREATE TABLE drifting_bottles_comments ( user_id   INTEGER, group_id   INTEGER, text   TEXT, time  INTEGER ); "
        )
        conn.commit()
        cur.execute(
            "INSERT INTO drifting_bottles_comments (user_id,group_id,text,time)VALUES (?,?,?,?);",
...
def load_comment(uuid: str):
    ...
    try:
        cur.execute(
            "SELECT user_id, group_id, text, time   FROM drifting_bottles_comments; "
        )
    except sqlite3.OperationalError:
        logging.info("数据库表不存在,正在创建表drifting_bottles_comments")
        cur.execute(
    ...
    if match:
        ...
        if len(a) == 0:
            ...
            if not HasKeyWords(raw_message, ["[CQ:image"]):
                ...
                await ReplySay(
                    websocket, group_id, message_id, f"评论ID为{uuid}的漂流瓶成功喵!"
                )
                return True
            else:
                return False
