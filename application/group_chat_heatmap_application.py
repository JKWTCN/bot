...
def _query_group_weekday_hour_activity(group_id: int, start_ts: int, end_ts: int):
    """
    查询群组在指定时间段内按星期+小时的发言次数分布
...
def _query_group_first_chat_datetime(group_id: int):
    """查询群组首次发言时间"""
    conn = sqlite3.connect("bot.db", timeout=30.0)
    cur = conn.cursor()
    cur.execute(
        "SELECT MIN(time) FROM group_message WHERE group_id=?",
        (group_id,),
    )
    row = cur.fetchone()
    conn.close()
...
def _draw_heatmap_scatter_by_base64(heatmap_data, title: str, show_annotations: bool = True, show_trendline: bool = False):
    ...
    # 保存并转换为base64
    output_path = f"figs/group_chat_heatmap_{int(time.time() * 1000)}.png"
...
async def SendGroupPeriodHeatmap(websocket, group_id: int, period: str, period_cn: str,
    ...
    if period == "life":
        first_chat_dt = _query_group_first_chat_datetime(group_id)
    ...
    # 查询活跃度数据
    activity_data = _query_group_weekday_hour_activity(group_id, start_ts, end_ts)
        ...
        payload["params"]["message"].append(
            {
                "type": "text",
                "data": {
                    "text": f"{group_name} 在{period_cn}暂无发言记录"
                },
            }
        )
    else:
        payload["params"]["message"].append(
            {
                "type": "text",
                "data": {
                    "text": f"{group_name} 的{period_cn}发言热力图（总发言: {total_count}）"
                },
            }
        )
...
class GroupChatHeatmapApplication(GroupMessageApplication):
    def __init__(self):
        applicationInfo = ApplicationInfo("群发言热力图", "查看群组的生涯/本年/本季度/本月/本周发言热力分布")
        # 设置优先级为 52（比个人热力图的 51 更高，数字越大优先级越高），确保群热力图先触发
        super().__init__(applicationInfo, 52, False, ApplicationCostType.NORMAL)
    ...
    def judge(self, message: GroupMessageInfo) -> bool:
        ...
        return HasKeyWords(plain_text, [load_setting("bot_name", "乐可")]) and (
            HasAllKeyWords(plain_text, ["群", "生涯", "发言", "热力"])
            or HasAllKeyWords(plain_text, ["群", "生涯", "发言", "热力图"])
            or HasAllKeyWords(plain_text, ["群", "本年", "发言", "热力"])
            or HasAllKeyWords(plain_text, ["群", "本年", "发言", "热力图"])
            or HasAllKeyWords(plain_text, ["群", "本季度", "发言", "热力"])
            or HasAllKeyWords(plain_text, ["群", "本季度", "发言", "热力图"])
            or HasAllKeyWords(plain_text, ["群", "本季", "发言", "热力"])
            or HasAllKeyWords(plain_text, ["群", "本季", "发言", "热力图"])
            or HasAllKeyWords(plain_text, ["群", "本月", "发言", "热力"])
            or HasAllKeyWords(plain_text, ["群", "本月", "发言", "热力图"])
            or HasAllKeyWords(plain_text, ["群", "本周", "发言", "热力"])
            or HasAllKeyWords(plain_text, ["群", "本周", "发言", "热力图"])
        )
