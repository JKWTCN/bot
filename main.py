                ...
                match message["message_type"]:
                    ...
                    case "group":
                        ...
                        if HasAllKeyWords(raw_message, ["乐可"]) and HasKeyWords(
                            ...
                            await robot_reply(websocket, user_id, group_id, message_id)
                        # 1% 的概率复读
                        ...
                        if "[CQ:at,qq=" in message["raw_message"]:
                            ...
                            if (
                                ...
                                if "解除禁言" in message["raw_message"]:
                                    ...
                                    if not IsAdmin(user_id, group_id):
                                        ...
                                        right_at = True
                                elif HasKeyWords(raw_message, ["晋升"]) and IsDeveloper(
                                    user_id
                                ):
                                    set_level(
                                        at_id,
                            ...
                            # 管理艾特乐可
                                ...
                                if "解除全体禁言" in message["raw_message"]:
                                    ...
                                    now_status = GetGroupDecreaseMessageStatus(group_id)
                                    sender_name = get_user_name(user_id, group_id)
                                    group_name = GetGroupName(group_id)
                                    logging.info(
                        ...
                        #     await websocket.send(json.dumps(payload))
                        if (
                            re.search(r"CQ:reply,id=\d+]好好好", message["raw_message"])
                        ...
                        elif re.search(r"CQ:reply,id=\d+]加精", message["raw_message"]):
                            message_id = re.findall(
                                r"CQ:reply,id=(\d+)", message["raw_message"]
                            )[0]
                                ...
                                case "text":
                                    ...
                                    if message["message"][0]["data"]["text"].startswith(
                                        ...
                                        if (
                                            ...
                                            await MemeStatistics(websocket, group_id)
                                        elif (
                                            "统计"
                                            in message["message"][0]["data"]["text"]
                                        ):
                                            ...
                                            await GetLifeChatRecord(websocket, group_id)
                                        elif (
                                            "水群排名"
                                            in message["message"][0]["data"]["text"]
                                        ):
                                            await GetNowChatRecord(websocket, group_id)
                                        elif (
                                            "排名"
                                            in message["message"][0]["data"]["text"]
                                        ):
                                            ...
                                            await drawing(websocket, user_id, group_id)
                                        elif (
                                            "抽"
                                            in message["message"][0]["data"]["text"]
                                            and "连"
                                            ...
                                            if user_id in setting["developers_list"]:
                                                result = re.search(
                                                    r"\d+",
                                                    message["raw_message"],
                                                )
                                            ...
                                            await send_random_meme(websocket, group_id)
                                            ...
                                            await get_cos(websocket, user_id, group_id)
                                        elif HasKeyWords(
                                            message["message"][0]["data"]["text"],
                                            ["打我"],
                                        ):
                                            ...
                                            await one_word(websocket, user_id, group_id)
                                            ...
                                            now_hour = datetime.datetime.now().strftime(
                                                "%H"
                                            )
                                        ...
                                        elif HasKeyWords(raw_message, ["切换思考显示"]):
            ...
            case "notice":
                ...
                match message["notice_type"]:
                    ...
                    case "group_increase":
                        ...
                        group_id = message["group_id"]
                        print(
                            "{}:{}加入入群{}".format(message["time"], user_id, group_id)
                        )
                        ...
                        case "heartbeat":
                            ...
                            for index, user in enumerate(setting["alarm_member"]):
                                if (
                                    ...
                                    or datetime.datetime.now().hour > user["time_hour"]
                                    ...
                                    setting["alarm_member"][index]["time"] = time.time()
                                    dump_setting(setting)
                                    setting = load_setting()
                        case _:
                            print(message)
